-- 013_fix_submit_exam.sql
-- Fix submit_exam: the previous loop iterated jsonb_array_elements into a
-- `jsonb` scalar variable and then tried to access `.value` on it, which
-- fails at runtime ("missing FROM-clause entry for table v_ans"). Iterate
-- into an explicit record instead.
