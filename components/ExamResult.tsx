...
export default function ExamResult({
      ...
      <div className="space-y-6">
        ...
        <section className="hud-card fade-in rounded-xl p-5 sm:p-6">
          ...
          <div className="space-y-3">
            {q.options.map((option, index) => {
              ...
              if (isCorrectAnswer) {
                ...
                classes +=
                  "border-cyan-300/15 bg-[#0f1730]/50 text-[#8299bc]";
              }
          ...
          <div className="mt-5 rounded-xl border border-violet-300/35 bg-violet-400/10 p-4 fade-in">
            ...
            <p className="mt-1 text-sm text-[#d4e4ff] sm:text-base">
              {q.explanation}
            </p>
          </div>
          ...
          <div className="mt-5 flex justify-between">
            <button
              ...
              className="rounded-xl border border-cyan-300/30 bg-[#141d35]/60 px-5 py-3 text-sm font-medium text-[#d8e5ff] transition-colors hover:border-cyan-300/40 hover:text-cyan-200 active:border-cyan-300/40 active:text-cyan-200 disabled:cursor-not-allowed disabled:opacity-40"
            >
              ← Previous
            </button>
            {reviewIndex < total - 1 ? (
              <button
                ...
                className="rounded-xl border border-cyan-300/30 bg-[#141d35]/60 px-5 py-3 text-sm font-medium text-[#d8e5ff] transition-colors hover:border-cyan-300/40 hover:text-cyan-200 active:border-cyan-300/40 active:text-cyan-200"
              >
                Next →
              </button>
            ) : (
  ...
  return (
    <section className="hud-card fade-in rounded-xl p-6">
      <h2 className="text-3xl font-bold text-[#f2f8ff]">Exam Complete</h2>
      <p className="mt-1 text-[#8ca3c5]">Here's how you performed.</p>
